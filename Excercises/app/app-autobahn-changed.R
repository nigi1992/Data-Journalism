...
# loading state-level administrative borders for Germany
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        inputId = "dayTypeFilter",
        label = "Select Day Type:",
        choices = c("All Days", "Weekdays", "Weekends"),
        selected = "All Days"
      ),
...
server <- function(input, output, session) {
  ...
  filteredData <- reactive({
    ...
    if (!is.null(input$dayTypeFilter) && nrow(df) > 0) {
      df$weekday <- weekdays(as.Date(df$startTimestamp))
      is_weekend <- df$weekday %in% c("Saturday", "Sunday")
      
      if (input$dayTypeFilter == "Weekends") {
        df <- df[is_weekend, ]
      } else if (input$dayTypeFilter == "Weekdays") {
        df <- df[!is_weekend, ]
      }
